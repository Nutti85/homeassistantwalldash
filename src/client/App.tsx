  ...
  return <main className="stitch-canvas">
    ...
    <div className="stitch-grid">
      ...
      <section className={`stitch-card guest-card ${guestOn ? 'guest-active' : ''}`} role="group" aria-label="Gjestemodus">
        ...
        <div className="card-bottom guest-bottom"><div><h2>Gjestemodus</h2></div><button type="button" className={`toggle ${guestOn ? 'toggle-on' : ''}`} role="switch" aria-checked={guestOn} aria-label={guestOn ? 'Slå av Gjestemodus' : 'Slå på Gjestemodus'} disabled={pending.guestMode} onClick={() => action('guestMode')}><span /></button></div>{errors.guestMode && <p role="alert">{errors.guestMode}</p>}
      </section>
      ...
      <button type="button" className="stitch-card scene-card" disabled={pending.night} onClick={() => action('night')}><div className="icon-tile scene-icon night"><Icon filled>bedtime</Icon></div><div className="card-bottom"><h2>Nattmodus</h2><p>Låser dører og skrur av lys etter 10 minutter</p></div>{errors.night && <span role="alert">{errors.night}</span>}</button>
      <section className="stitch-card climate-card" role="group" aria-label="Automatisk Kjøling"><div className="climate-content"><div className="climate-info"><div className="climate-heading"><div className="icon-tile climate-icon"><Icon>ac_unit</Icon></div><div><h2>Automatisk Kjøling</h2></div></div><div className="climate-action"><button type="button" className={`toggle ${coolingOn ? 'toggle-on' : ''}`} role="switch" aria-checked={coolingOn} aria-label={coolingOn ? 'Slå av Automatisk kjøling' : 'Slå på Automatisk kjøling'} disabled={pending.cooling} onClick={() => action('cooling')}><span /></button><span role="status" aria-label="Automatisk Kjøling status">Automatisk kjøling</span></div>{errors.cooling && <p role="alert">{errors.cooling}</p>}</div><div className="temperature-panel"><span>Måltemperatur</span><div className="stepper"><button type="button" aria-label="Senk temperatur" disabled={pending.temperature || temperatureBaseline === undefined} onClick={() => adjustTemperature(-1)}><Icon>remove</Icon></button><output aria-label="Temperatur">{targetTemperature ?? '—'}<small>°C</small></output><button type="button" aria-label="Øk temperatur" disabled={pending.temperature || temperatureBaseline === undefined} onClick={() => adjustTemperature(1)}><Icon>add</Icon></button></div><p>Inne: <strong>{indoorReading}</strong></p>{targetTemperature === undefined && <em>Trykk ± for å aktivere kjøling og sette måltemperatur</em>}{errors.temperature && <p role="alert">{errors.temperature}</p>}</div></div></section>
    </div>
