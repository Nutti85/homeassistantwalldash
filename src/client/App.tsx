  ...
  return <main className="stitch-canvas">
    ...
    <div className="stitch-grid">
      ...
      <section className={`stitch-card guest-card ${guestOn ? 'guest-active' : ''}`} role="group" aria-label="Gjestemodus">
        ...
        <div className="card-bottom guest-bottom"><div><h2>Gjestemodus</h2><p>Begrens tilgang</p></div><button type="button" className={`toggle ${guestOn ? 'toggle-on' : ''}`} role="switch" aria-checked={guestOn} aria-label="Slå på Gjestemodus" disabled={pending.guestMode || guestOn} onClick={() => action('guestMode')}><span /></button></div>{errors.guestMode && <p role="alert">{errors.guestMode}</p>}
      </section>
